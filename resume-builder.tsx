...
export function ResumeBuilder({ selectedTemplate, onPreview, onBack }: ResumeBuilderProps) {
  ...
  // Check if resume is complete (all required fields filled)
  const isResumeComplete = () => {
    // Check personal info
    if (!formData.personal.name.trim()) return false;
    if (!formData.personal.title.trim()) return false;
    if (!formData.personal.email.trim()) return false;
    if (!validateEmail(formData.personal.email)) return false;
    if (!formData.personal.phone.trim()) return false;
    if (!validatePhone(formData.personal.phone)) return false;
    if (!formData.personal.location.trim()) return false;
    if (!formData.personal.summary.trim()) return false;
    
    // Check education
    if (formData.education.length === 0) return false;
    for (const edu of formData.education) {
      if (!edu.degree.trim() || !edu.school.trim() || !edu.year.trim()) return false;
      if (!validateNumericYear(edu.year)) return false;
    }
    
    // Check experience
    if (formData.experience.length === 0) return false;
    for (const exp of formData.experience) {
      if (!exp.role.trim() || !exp.company.trim() || !exp.duration.trim()) return false;
    }
  ...
  const handleSave = () => {
    ...
    // Save current resume data for preview/download
    localStorage.setItem('resumeData', JSON.stringify(formData));
    
    // Only save to dashboard if resume is complete
    if (!isResumeComplete()) {
      toast.success("Resume saved! Complete all required fields to see it on the Dashboard.");
      return;
    }
    
    // Save to the savedResumes array for dashboard
    try {
      const stored = localStorage.getItem('savedResumes');
      let resumes = stored ? JSON.parse(stored) : [];
      
      // Use existing resume ID or create new one
      const resumeId = currentResumeId || `resume_${Date.now()}`;
      
      // Template preview images
      const templateImages: Record<string, string> = {
        classic: "https://images.unsplash.com/photo-1693045181676-57199422ee66?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxyZXN1bWUlMjB0ZW1wbGF0ZSUyMGRvY3VtZW50fGVufDF8fHx8MTc2MDQyOTk5NHww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
        modern: "https://images.unsplash.com/photo-1610650394144-a778795cf585?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwcm9mZXNzaW9uYWwlMjBjdiUyMGxheW91dHxlbnwxfHx8fDE3NjA0Mjk5OTR8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
        creative: "https://images.unsplash.com/photo-1693045181229-3eddc1a2e242?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtaW5pbWFsaXN0JTIwcmVzdW1lJTIwZGVzaWdufGVufDF8fHx8MTc2MDQyOTk5NXww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral"
      };
      
      // Create resume metadata
      const resumeData = {
        id: resumeId,
        title: `${formData.personal.name || 'Untitled'} - ${formData.personal.title || 'Resume'}`,
        lastModified: new Date().toISOString(),
        template: currentTemplate || 'Classic Layout',
        previewUrl: templateImages[currentTemplate] || templateImages.classic,
        data: formData,
        isComplete: true
      };
      
      // Check if resume with this ID already exists
      const existingIndex = resumes.findIndex((r: any) => r.id === resumeId);
      ...
      toast.error("Error saving resume to dashboard");
    }
  };
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-5 space-y-6">
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-bold text-[#47297B]" style={{ fontFamily: 'Poppins, sans-serif' }}>Resume Builder</h1>
              <Button 
                variant="outline" 
                onClick={onBack}
                className="border-[#8736AA] text-[#8736AA] hover:bg-[#8736AA] hover:text-white normal-case"
            ...
            <Tabs defaultValue="personal" className="w-full">
              ...
              <TabsContent value="personal" className="space-y-4">
                <Card>
                  <CardHeader>
                    <h3 className="font-semibold text-[#5D2689]" style={{ fontFamily: 'Poppins, sans-serif' }}>Personal Information</h3>
                  </CardHeader>
            ...
            <div className="flex space-x-4 pt-4 border-t">
              <Button 
                onClick={handleSave} 
                className="bg-[#8736AA] hover:bg-[#5D2689] text-white normal-case"
                style={{ fontFamily: 'Poppins, sans-serif' }}
              >
              ...
              <Button 
                variant="outline" 
                onClick={handleReset}
                className="border-[#EF4444] text-[#EF4444] hover:bg-[#EF4444] hover:text-white normal-case"
                style={{ fontFamily: 'Poppins, sans-serif' }}
              >
              ...
              <Button 
                ...
                className="border-[#F5BD1F] text-[#F5BD1F] hover:bg-[#F5BD1F] hover:text-white normal-case"
                style={{ fontFamily: 'Poppins, sans-serif' }}
              >
          ...
          <div className="col-span-7 space-y-6">
            ...
            {/* A4 Preview Area */}
            <div className="bg-white rounded-lg shadow-lg p-8 mx-auto mb-8" style={{ width: '595px', minHeight: '842px' }}>
            ...
            <div className="flex justify-center space-x-4 pb-8">
              <Button 
                onClick={handleSave} 
                variant="outline"
                className="border-[#8736AA] text-[#8736AA] hover:bg-[#FFE863] hover:border-[#F5BD1F] normal-case"
                style={{ fontFamily: 'Poppins, sans-serif' }}
              >
                <Save className="w-4 h-4 mr-2" />
